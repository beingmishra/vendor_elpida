...
func init() {
	android.RegisterModuleType("elpida_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = elpidaExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = elpidaExpandVariables(ctx, dep_file)
	...
	cmd := elpidaExpandVariables(ctx, String(g.properties.Cmd))
