...
function setup_vendor() {
    ...
    export ELPIDA_ROOT="$3"
    if [ ! -d "$ELPIDA_ROOT" ]; then
        echo "\$ELPIDA_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$ELPIDA_ROOT/$OUTDIR" ]; then
        mkdir -p "$ELPIDA_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$ELPIDA_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$ELPIDA_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$ELPIDA_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$ELPIDA_ROOT"/vendor/elpida/build/tools/smali/baksmali.jar
        export SMALIJAR="$ELPIDA_ROOT"/vendor/elpida/build/tools/smali/smali.jar
    fi
    ...
    if [ ! -f "$ELPIDA_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$ELPIDA_TARGET" >/dev/null; then
...
function extract() {
    ...
    local SRC="$2"
    local OUTPUT_ROOT="$ELPIDA_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        DUMPDIR="$ELPIDA_ROOT"/system_dump
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$ELPIDA_ROOT"/vendor/elpida/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$ELPIDA_ROOT"/"$OUTDIR"/radio
